...
تاریخ باید در عنوان باشد. اگر تاریخ نباشد، تاریخ جلسه یا تاریخ اکنون در نظر گرفته می‌شود.
تاریخ‌ها باید به صورت عددی نوشته شود، به عنوان نمونه: ۱۴۰۴/۵/۶

مصوبات:
در خط‌های بعدی آورده شود و هر مصوبه در یک خط جداگانه که با - یا _ شروع شود.
توی هر خط می‌توان از یک @ برای تعیین مسئول پیگیری آن کار استفاده کرد، مثل @خیری یا @قدسیه یا @طلبی یا @طالبی و...
(باید توجه داشت که در مصوباتی که دستگاه اجرایی مسئول انجام آن است، @ به معنای فردی است که باید پیگیری کند آن کار توسط ارگان مربوطه انجام شده است یا خیر.)
اگر تعریف نشود، شخصی که صورت جلسه را فرستاده به عنوان پیگیری‌کننده در نظر گرفته می‌شود.
...
امضا ها:
باید اسم ارگان مربوطه با @ در انتهای توضیحات اضافه گردد و به جای فاصله بین کلمات یک ارگان باید از _ استفاده کرد.
به عنوان مثال اگر بخواهیم دو امضا کننده "راه و شهرسازی استان" و "اداره راهداری استان" را داشته باشیم باید به صورت زیر اضافه کنیم:
@راه_و_شهرسازی_استان @راهداری_استان

📎 نکته:
باید خبر جلسه قبل از بارگزاری صورت جلسه در کانال ایتای جناب آقای حاجی بارگزاری شده باشد تا صورت جلسه ضمیمه آن جلسه شود.

✅ /صورتجلسه
این دستور ۵ صورت جلسه آخر مربوط به کاربر احراز هویت شده را ارسال می‌کند.
می‌توان با یک فاصله بعد از دستور، عنوان صورتجلسه یا شماره صورتجلسه را وارد کرد تا با استفاده از آن در صورت‌جلسه‌ها جستجو شود.
TEXT;
...
/راهنما
این دستور راهنمای ربات را ارسال می‌کند. در ادامه دستور، عنوان هر موجودیت شامل کار، نامه، صورت‌جلسه و... را وارد کنید تا راهنمای آن موجودیت برای شما ارسال شود.

✅ لیست دستورات ثبتی:
#صورتجلسه
ایجاد صورت‌جلسه
#کار
ایجاد کار، شمار جلسه، مصوبه، پیگیری، بازدید و ...
#نامه
ایجاد نامه

⚠️ توجه!
ربات به فاصله‌ها (اسپیس یا فضای خالی) بین کلمات و دستورات حساس می‌باشد.
به عنوان مثال: ( # صورت جلسه ) یا ( / کار ) اشتباه است.
شکل صحیح آن: ( #صورتجلسه ) یا ( /کار ) می‌باشد.
TEXT;
